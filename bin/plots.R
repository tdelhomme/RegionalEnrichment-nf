...
for(cl in celllines){
  ...
  allsm = unlist(lapply(dat_cl, function(x) unlist(strsplit(gsub(".Rdata", "", x), paste(cl, "_", sep="")))[3]))
  for(sm in allsm){
    load(paste(cl, "_", cl, "_", sm, ".Rdata", sep=""))
  ...
  dd[which(grepl("HE", dd$sm)), "type"] = "Helium"
  dd[which(grepl("PR", dd$sm)), "type"] = "Proton"
